...
def fs_processor(main_dict: dict) -> None:
    ...
    tag_dict = tag_dict_options[str(main_dict["processing_dict"]["tag_set"])]
    ...
    while i < 3:
        for item in tag_pairs:
            main_dict["control_info"]["output_text"] = \
                        main_dict["control_info"]["output_text"].\
                        replace(item, "")
        i += 1

    # Create a backup of the output_file.xml.
    main_dict["control_info"]["output_text_bak"] = \
        main_dict["control_info"]["output_text"]
    output_file = os.path.join(main_dict["control_info"]["debug_dir"],
                               "output_file.xml")
    with open(output_file, "r+") as output_file_pre:
        output_file_pre.write(main_dict["control_info"]["output_text"])
    ...
    xml_formatted_text = xmlformatter.xmlformatter_start(
        ...
        outfile=main_dict["control_info"]["output_file_name"])

    if xml_formatted_text is tuple:
        print("Tuple")
        sys.exit(0)
    else:
        with open(main_dict["control_info"]["output_file_name"], "w+") \
    ...
    final_file = ntpath.basename(main_dict["control_info"]["output_file_name"])
    output_dir = os.path.join(main_dict["control_info"]["base_dir"], "output")

    copy(main_dict["control_info"]["output_file_name"], output_dir)
